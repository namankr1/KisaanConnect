...
public class UserNegotiationAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if(notifstatus==1 || notifstatus==2){
            if(notifstatus==1){
                textView.setText("You are negotiating with "+notificationsendername[position]+" for "+notificationtype[position]+", curently for " +notificationquantity[position]+" kg at rate Rs. "+notificationprice[position]+" ");
            }
            else{
                textView.setText("You are negotiating with "+notificationsendername[position]+" for "+notificationtype[position]+", curently for " +notificationquantity[position]+" kg at rate Rs. "+notificationprice[position]+" ");
            }
